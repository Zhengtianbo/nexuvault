#!/bin/bash
# Update VaultContext.tsx with tx confirmation fix

cd ~/Desktop/app

# Write the fixed VaultContext.tsx
cat > src/hooks/VaultContext.tsx << 'PYEOF'
...
export function VaultProvider({ children }: { children: ReactNode }) {
  ...
  const approveToken = useCallback(async (tokenAddress: Address, amount: bigint) => {
    ...
    try {
      ...
      setTxState({ status: 'approving', hash }); addTx('approve', sym, 'Unlimited')
      return hash
    } catch (err: any) { setTxState({ status: 'error', error: err.message }); throw err }
  }, [useMock, writeContractAsync, publicClient])
  ...
  }, [useMock, writeContractAsync, pools, refetchUserInfo, refetchBalances, publicClient])
  ...
  }, [useMock, writeContractAsync, pools, mockRewards, refetchUserInfo, publicClient])
  ...
  }, [useMock, writeContractAsync, refetchBalances, publicClient])
...
# Update PoolCard.tsx - fix localTx clear on deposit
sed -i '' 's/if (!useMock) setLocalTx/\/\/ cleared after deposit success\n          setLocalTx/' src/components/PoolCard.tsx

echo "Files updated! Restarting dev server..."
